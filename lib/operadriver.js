...
process.env.PATH = path.join(__dirname, "operadriver") + path.delimiter + process.env.PATH;
exports.path =
  process.platform === "win32"
    ? path.join(__dirname, "operadriver", "operadriver.exe")
    : path.join(__dirname, "operadriver", "operadriver");
exports.version = "116.0.5845.97";
exports.start = function (args, returnPromise) {
  let command = exports.path;
  if (!fs.existsSync(command)) {
    console.log("Could not find operadriver in default path: ", command);
    console.log("Falling back to use global operadriver bin");
    command = process.platform === "win32" ? "operadriver.exe" : "operadriver";
  }
